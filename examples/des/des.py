...
if __name__ == "__main__":
    ...
    # IP
    IP = tikz.rectangle_from_north(
        to_IP.end, height=0.7, width=2, options="rounded corners"
    )  # Rectangle who's .north position is the end of IP
    ...
    def DES_round(
        ...
        to_next_right_block = tikz.line(
            XOR_diagonal.end, XOR_diagonal.end - (0, 0.25), options="->"
        )
        ...
        to_next_left_block = tikz.line(
            dot_diagonal.end, dot_diagonal.end - (0, 0.25), options="->"
        )
        ...
        if K_i is not None:
            joint = tikz.line(
                from_dot.midpoint() + (1, -0.2),
                (to_f.midpoint().x, from_dot.midpoint().y - 0.2),
            )
    ...
    next_left_block, next_right_block = DES_round(
        to_left_block, to_right_block, "$L_0$", "$R_0$", "$K_1$"
    )
    next_left_block, next_right_block = DES_round(
        next_left_block,
        next_right_block,
        "$L_1=R_0$",
        r"$R_1 = L_0 \oplus f(R_0, K_1)$",
        "$K_2$",
    )
    next_left_block, next_right_block = DES_round(
        next_left_block,
    ...
    next_left_block, next_right_block = DES_round(
        ...
        next_right_block,
        "$L_{15}=R_{14}$",
        r"$R_{15} = L_{14} \oplus f(R_{14}, K_{15})$",
        "$K_{16}$",
        is_last_block=True,
    )
    ...
    tikz.node(L_0.center, text=r"$R_{16} = L_{15}\oplus f(R_{15}, K_{16})$")
    ...
    IP_inv = tikz.rectangle_from_north(
        to_IP_inv.end, height=0.7, width=2, options="rounded corners"
    )
