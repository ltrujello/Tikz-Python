...
class Path:
    ...
    def __str__(self):
        ...
        for k in range(L - last):
            ...
            r.append(
                "(%.4f, %.4f) .. controls (%.5f, %.5f) and (%.5f, %.5f)"
    ...
    def __repr__(self):
        ...
        r = ["Path information"]
        r.append(
            "Cyclic=%s, curl_begin=%s, curl_end=%s"
            % (self.cyclic, self.curl_begin, self.curl_end)
        )
        for pt in self.p:
            r.append(str(pt))
        return "\n".join(r)
...
# "Velocity" function
def f(theta, phi):
    n = 2 + sqrt(2) * (sin(theta) - sin(phi) / 16) * (sin(phi) - sin(theta) / 16) * (
        cos(theta) - cos(phi)
    )
